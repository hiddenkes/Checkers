...
function outputMove(move) {
    console.log("Choosing move from " + move.piece.x + "," + move.piece.y + " to " + move[0] + "," + move[1]);
    if (move.safe)
        console.log("Move is safe");
    else
        console.log("Move is not safe");
}
...
function performMove(checkerBoard, move) {
    ...
    if (checkerBoard.chainJumper)
        window.setTimeout(performDeferredMove(checkerBoard, move), 500);
    else
